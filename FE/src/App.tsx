...
export default function App() {
  ...
  const handleInsertVariable = useCallback((variable: string) => {
    ...
    if (ta) {
      ...
      campaign.setHtmlBody(
        campaign.htmlBody.substring(0, start) + `{{${variable}}}` + campaign.htmlBody.substring(end),
      );
      setTimeout(() => { ta.focus(); ta.selectionStart = ta.selectionEnd = start + variable.length + 4; }, 0);
    }
  }, [campaign]);
  ...
  if (!user) return <LoginPage loading={loading} error={authError} onSignIn={signIn} />;

  return (
    <div className="min-h-screen bg-white text-midnight-ink flex flex-col font-sans">
      <AppHeader
        user={user}
      ...
      <main className="flex-1 px-10 pb-[100px] pt-10 max-w-[1280px] w-full mx-auto max-md:px-5 max-md:pb-20 max-md:pt-6">
        {page === 'dashboard' && (
          <DashboardPage onNewCampaign={goToWizard} onViewDetail={goToDetail} />
        )}
        ...
        {page === 'wizard' && (
          <>
            <CampaignStepper current={campaign.step} hidden={campaign.step === 4} />
            {campaign.step !== 4 && (
              <div className="flex items-center gap-3 mb-8">
                ...
                />
                <Edit2 size={16} color="#707070" style={{ cursor: 'pointer' }} />
              </div>
            )}
